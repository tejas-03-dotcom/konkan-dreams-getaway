      ...
      <div className="container">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
          <div className="max-w-2xl">
            ...
            <h2 className="text-4xl md:text-6xl font-bold mt-3">
              {location ? <>Stays in <span className="text-gradient-sunset">{location}</span></> : "Handpicked Konkan stays"}
            </h2>
          </div>
        ...
        {location && (
          <div className="mb-8 flex items-center gap-3 flex-wrap">
            ...
            <span className="text-sm text-muted-foreground">{filtered.length} stay{filtered.length !== 1 && "s"} found</span>
          </div>
        )}

        {filtered.length === 0 ? (
          <div className="text-center py-20 rounded-3xl bg-secondary/40">
            <p className="text-lg text-muted-foreground mb-4">No stays listed in {location} yet.</p>
            <Button variant="outline" onClick={() => setLocation(null)}>Show all stays</Button>
          </div>
        ) : (
