      ...
      <div className="container">
        <div className="max-w-2xl mb-8">
          ...
          <h2 className="text-4xl md:text-6xl font-bold mt-3 mb-4">
            {location ? <>Guides for <span className="text-gradient-sunset">{location}</span></> : "Travel with people who call Konkan home"}
          </h2>
        ...
        {location && (
          <div className="mb-8 flex items-center gap-3 flex-wrap">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/15 text-accent text-sm font-semibold">
              <MapPin className="w-4 h-4" /> Filtered by {location}
              <button onClick={() => setLocation(null)} aria-label="Clear filter" className="ml-1 hover:bg-accent hover:text-accent-foreground rounded-full p-0.5 transition-smooth">
                <X className="w-3.5 h-3.5" />
              </button>
            </span>
            <span className="text-sm text-muted-foreground">{filtered.length} guide{filtered.length !== 1 && "s"} found</span>
          </div>
        )}

        {filtered.length === 0 ? (
          <div className="text-center py-20 rounded-3xl bg-card/60">
            <p className="text-lg text-muted-foreground mb-4">No guides registered for {location} yet.</p>
            <Button variant="outline" onClick={() => setLocation(null)}>Show all guides</Button>
          </div>
        ) : (
